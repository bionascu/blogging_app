...
class EditProfileAdminForm(Form):
	...
	def __init__(self, user, *args, **kwargs):
		...
		# set items of the SelectField instance in its choices attribute
		self.role.choices = [(role.id, role.name) for role in 
			Role.query.order_by(Role.name).all()]
		self.user = user # the constructor receives the user as an argument and
						# saves it as a member variable, which is later used in 
